...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Filter and sort using the utility function
    const currentDate = new Date(); // or pass a specific date
    const relevantDocuments = deletePreviousDayShowsSevenComingEvents(
      formattedDocuments,
      currentDate
    );

    if (relevantDocuments.length > 0) {
      return NextResponse.json(relevantDocuments);
    } else {
      return NextResponse.json(
